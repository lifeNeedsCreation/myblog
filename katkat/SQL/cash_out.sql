with
    ...
    select country_code, paid_out_date, sum(usd_value) as money from pay_out_log inner join accounts on account_id = id where paid_out_date >= "{start_time}" and paid_out_date < "{end_time}"  group by country_code, paid_out_date order by country_code, paid_out_date