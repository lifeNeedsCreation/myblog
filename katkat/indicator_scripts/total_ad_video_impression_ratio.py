...
class TotalAdVideoImpressionRatio(object):
    """
    ...
    def compute_data(self, path):
        ...
        total_ad_video_impression_ratio_data = self.get_data(query)
        if total_ad_video_impression_ratio_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(total_ad_video_impression_ratio_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{total_ad_video_impression_ratio_data[field][i]}', "
