...
class NotificationVideoCtrWithoutExperimentsByPeople(object):
    '''
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ['country_code', 'date', 'click_num', 'received_num', 'ctr']
    ...
    def compute_data(self, path):
        ...
        notification_video_ctr_without_experiments_by_people_data = self.get_data(query)
        if notification_video_ctr_without_experiments_by_people_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(notification_video_ctr_without_experiments_by_people_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{notification_video_ctr_without_experiments_by_people_data[field][i]}", '
