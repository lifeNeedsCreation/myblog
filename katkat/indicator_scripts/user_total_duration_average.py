...
class UserTotalDurationAverage(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "date", "sum_duration", "user_num", "avg_duration"]
    ...
    def compute_data(self, path):
        ...
        user_total_duration_average_data = self.get_data(query)
        if user_total_duration_average_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(user_total_duration_average_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{user_total_duration_average_data[field][i]}', "
