...
class CashOutByCreatedAt(object):
    """
    ...
    def __init__(self, start_time, end_time, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "paid_out_date", "payout_method", "money"]
    ...
    def compute_data(self, path):
        ...
        cashout_by_created_at_data = self.get_data(query)
        if cashout_by_created_at_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(cashout_by_created_at_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{cashout_by_created_at_data[field][i]}', "
