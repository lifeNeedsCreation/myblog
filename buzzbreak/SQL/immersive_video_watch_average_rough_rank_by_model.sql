with
    ...
    video_watch_info as (select account_id, json_extract_scalar(data, "$.placement") as placement, json_extract_scalar(meta_tag, "$.ranking_bucket") as bucket, json_extract_scalar(meta_tag, "$.ranking_strategy") as strategy, json_extract_scalar(data, "$.id") as video_id from (select *, json_extract_scalar(data, "$.meta_tag") as meta_tag from stream_events.video_watch where created_at >= "{start_time}" and created_at < "{end_time}") where json_extract_scalar(meta_tag, "$.ranking_bucket") is not null),
    ...
    video_impression_info as (select account_id, json_extract_scalar(data, "$.placement") as placement, json_extract_scalar(meta_tag, "$.ranking_bucket") as bucket, json_extract_scalar(meta_tag, "$.ranking_strategy") as strategy, json_extract_scalar(data, "$.id") as video_id from (select *, json_extract_scalar(data, "$.meta_tag") as meta_tag from stream_events.video_impression where created_at >= "{start_time}" and created_at < "{end_time}") where json_extract_scalar(meta_tag, "$.ranking_bucket") is not null),
    ...
    video_impression_union as (select distinct account_id, placement, bucket, strategy from video_watch_info union distinct select distinct account_id, placement, bucket, strategy from video_impression_info),
