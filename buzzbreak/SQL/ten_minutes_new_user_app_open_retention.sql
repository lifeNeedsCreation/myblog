with
    ...
    user_time_group as (select distinct account_id, date from (select account_id, page, date, sum(duration_in_seconds) as page_time from user_time group by account_id, page, date) where page_time >= 10*60),
    ...
    immersive_user_time_group as (select distinct account_id, date from (select account_id, date, sum(duration_in_seconds) as page_time from immersive_user_time group by account_id, date) where page_time >= 10*60),
    ...
    news_read_group as (select distinct account_id, date from (select account_id, date, sum(duration_in_seconds) as news_read_time from news_read group by account_id, date) where news_read_time >= 10*60),
    ...
    video_watch_group as (select distinct account_id, date from (select account_id, date, sum(duration_in_seconds) as video_watch_time from video_watch group by account_id, date) where video_watch_time >= 10*60),
