with
    ...
    user_time_all_group as (select * from (select (case when (a.account_id is null) then v.account_id when (v.account_id is null) then a.account_id else a.account_id end) as account_id, (case when (a.date is null) then v.date when (v.date is null) then a.date else a.date end) as date, ifnull(page_news_time, 0) as page_page_news_timetime, ifnull(video_watch_time, 0) as video_watch_time, (ifnull(page_news_time, 0) + ifnull(video_watch_time, 0)) as total_use_time from (select (case when (u.account_id is null) then n.account_id when (n.account_id is null) then u.account_id else u.account_id end) as account_id, (case when (u.date is null) then n.date when (n.date is null) then u.date else u.date end) as date, ifnull(page_time, 0) as page_time, ifnull(news_read_time, 0) as news_read_time, (ifnull(page_time, 0) + ifnull(news_read_time, 0)) as page_news_time from user_time_group as u full outer join news_read_group as n on u.account_id = n.account_id and u.date = n.date) as a full outer join video_watch_group as v on a.account_id = v.account_id and a.date = v.date) where total_use_time >= 10*60),
