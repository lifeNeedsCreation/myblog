with
    ...
    accounts_point_out_info as (select country_code, p.*, json_extract_scalar(data, '$.purpose') as purpose from point_info_out as p inner join accounts on account_id = id),

    points_out_group as (select country_code, date, (case when purpose is null then 'others' else purpose end) as purpose, sum(amount) as points from accounts_point_out_info group by country_code, date, purpose),
