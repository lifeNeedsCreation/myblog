with
    ...
    select t.country_code as country_code, t.experiment as experiment, t.strategy as strategy, extract(date from timestamp('{start_time}')) as date, experiment_sum_duration, user_num, round(experiment_sum_duration/user_num, 2) as avg_duration from experiment_user_time as t inner join experiment_user_count as c on t.country_code = c.country_code and t.experiment = c.experiment and t.strategy = c.strategy order by experiment, strategy, avg_duration desc