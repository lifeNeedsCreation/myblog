with
    ...
    video_impression_info as (select account_id, json_extract_scalar(data, "$.placement") as placement, json_extract_scalar(data, "$.id") as video_id, json_extract_array(meta_tag, "$.experiment") as experiments, json_extract_array(meta_tag, "$.recall_strategy") as strategies from (select *, json_extract_scalar(data, "$.meta_tag") as meta_tag from stream_events.video_impression where created_at >= "{start_time}" and created_at < "{end_time}" and json_extract_scalar(data, "$.placement") not like "immersive%" and safe_cast(json_extract_scalar(data, "$.data_index") as numeric) = 0)),
