with
    ...
    video as (select id, category, video_length_seconds from partiko.videos where video_length_seconds < 60*1 and category is not null and created_at > timestamp_sub(timestamp'{start_time}', interval 7 day)),
    ...
    video_watch_update as (select account_id, video_id, (case when placement in ("immersive_videos_tab_popular", "immersive_videos_tab_home", "immersive_videos_tab_home_tab_home_video", "immersive_videos_tab_news_detail_activity", "immersive_videos_tab_home_tab_for_you_video") then "immersive_videos_tab_popular" else placement end) as placement, duration_in_seconds, date from video_watch),
