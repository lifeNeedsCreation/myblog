with
    ...
    click_group as (select country_code, date, count(distinct account_id) as click_num from accounts_click group by country_code, date),

    received_group as (select country_code, date, count(distinct account_id) as received_num from accounts_received group by country_code, date)
