with
    ...
    video_impression_info as (select account_id, json_extract_array(meta_tag, '$.experiment') as experiment, json_extract_array(meta_tag, '$.ranking_strategy') as rank_strategies from (select *, json_extract_scalar(data, '$.meta_tag') as meta_tag from stream_events.video_impression where created_at >= '{start_time}' and created_at < '{end_time}')),

    video_impression as (select account_id, replace(experiment, '"', '') as experiment, replace(rank_strategy, '"', '') as strategy from video_impression_info as v 
    ...
    rank_user_time_total as (select *, ROW_NUMBER() OVER (PARTITION BY country_code, experiment, strategy ORDER BY sum_duration) as rank from account_experiment_user_time_total),

    valid_experiment_user_time_total as (select * from (select r.country_code as country_code, r.account_id as account_id, r.experiment as experiment, r.strategy as strategy, r.sum_duration as sum_duration, rank, max_rank from rank_user_time_total as r inner join (select country_code, experiment, strategy, max(rank) as max_rank from rank_user_time_total group by country_code, experiment, strategy) as a on r.country_code = a.country_code and r.experiment = a.experiment and r.strategy = a.strategy) where rank BETWEEN max_rank*0.05 AND max_rank*0.95),

    experiment_user_time as (select country_code, experiment, strategy, sum(sum_duration) as experiment_sum_duration from valid_experiment_user_time_total group by country_code, experiment, strategy),

    experiment_user_count as (select country_code, experiment, strategy, count(account_id) as user_num from valid_experiment_user_time_total group by country_code, experiment, strategy)
