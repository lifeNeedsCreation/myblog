with
    ...
    select country_code, type, date, count(distinct account_id) as user_num from posts inner join accounts on account_id = id group by country_code, type, date order by country_code, type, date