with
    ...
    video_click as (select distinct country_code, account_id, json_extract_scalar(data, '$.placement') as placement from stream_events.video_click inner join accounts on account_id = id where created_at >= '{start_time}' and created_at < '{end_time}'),

    video_click_update as (select distinct country_code, account_id, (case when placement in ('home_tab_for_you', 'home_tab_home') then 'home_tab_for_you' when placement in ('home_tab_for_you_video', 'home_tab_home_video') then 'home_tab_for_you_video' else placement end) as placement from video_click),
    ...
    video_impression as (select distinct country_code, account_id, json_extract_scalar(data, '$.placement') as placement from stream_events.video_impression inner join accounts on account_id = id where created_at >= '{start_time}' and created_at < '{end_time}'),

    video_impression_update as (select distinct country_code, account_id, (case when placement in ('home_tab_for_you', 'home_tab_home') then 'home_tab_for_you' when placement in ('home_tab_for_you_video', 'home_tab_home_video') then 'home_tab_for_you_video' else placement end) as placement from video_impression),
    ...
    video_click_union_impression_update as (select distinct country_code, account_id, (case when placement in ('home_tab_for_you', 'home_tab_home') then 'home_tab_for_you' when placement in ('home_tab_for_you_video', 'home_tab_home_video') then 'home_tab_for_you_video' else placement end) as placement from video_click_union_impression),
