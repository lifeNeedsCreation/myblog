with
    ...
    accounts_video_click as (select account_id, country_code, placement, video_id, experiment, strategy from video_click inner join accounts on account_id = id where experiment != "cold_start_video_recall"),

    video_click_group as (select country_code, placement, "video_multiple_recommendation_model_v4" as experiment, strategy, count(distinct account_id) as click_num from accounts_video_click group by country_code, placement, strategy),
    ...
    accounts_video_impression as (select account_id, country_code, placement, video_id, experiment, strategy from video_impression inner join accounts on account_id = id where experiment != "cold_start_video_recall"),

    video_impression_group as (select country_code, placement, "video_multiple_recommendation_model_v4" as experiment, strategy, count(distinct account_id) as impression_num from accounts_video_impression group by country_code, placement, strategy),

    video_impression_union as (select * from video_click union distinct select * from video_impression),

    accounts_video_impression_union as (select account_id, country_code, placement, video_id, experiment, strategy from video_impression_union inner join accounts on account_id = id where experiment != "cold_start_video_recall"),

    video_impression_union_group as (select country_code, placement, "video_multiple_recommendation_model_v4" as experiment, strategy, count(distinct account_id) as impression_union_num from accounts_video_impression_union group by country_code, placement, strategy)
