...
class PageAvgTime(object):
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ['country_code', 'date', 'page', 'total_time', 'user_count', 'avg_time']
    ...
    def compute_data(self, path):
        ...
        page_avg_time_data = self.get_data(query)
        if page_avg_time_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(page_avg_time_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{page_avg_time_data[field][i]}", '
