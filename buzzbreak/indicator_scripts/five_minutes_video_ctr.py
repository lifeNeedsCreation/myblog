...
class FiveMinutesVideoCtr(object):
    """
    ...
    def compute_data(self, path):
        ...
        five_minutes_video_ctr_data = self.get_data(query)
        if five_minutes_video_ctr_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(five_minutes_video_ctr_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{five_minutes_video_ctr_data[field][i]}', "
