...
class VideoAverageOfTotalDurationByDimensionRecall(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, experiments, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "experiment", "dimension", "date", "experiment_sum_duration", "user_num", "avg_duration"]
    ...
    def compute_data(self, path):
        ...
        duration_avg_by_dimension_recall_data = self.get_data(query)
        if duration_avg_by_dimension_recall_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(duration_avg_by_dimension_recall_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{duration_avg_by_dimension_recall_data[field][i]}', "
