...
class FiveMinutesNewVideoCtr(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "date", "category", "placement", "click_num", "impression_num", "impression_union_num", "ctr", "ctr_union"]
    ...
    def compute_data(self, path):
        ...
        five_minutes_new_video_ctr_data = self.get_data(query)
        if five_minutes_new_video_ctr_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(five_minutes_new_video_ctr_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{five_minutes_new_video_ctr_data[field][i]}', "
