...
class PointsOutStatistics(object):
    """
    ...
    def compute_data(self, path):
        ...
        points_out_statistics_data = self.get_data(query)
        if points_out_statistics_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(points_out_statistics_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{points_out_statistics_data[field][i]}', "
