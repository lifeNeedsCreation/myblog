...
class ManualNewsPush(object):
    ...
    def __init__(self, start_time, end_time, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "date", "news_id", "click_num", "received_num", "click_ratio", "pushed_num", "received_ratio"]
    ...
    def compute_data(self, path):
        ...
        manual_news_push_data = self.get_data(query)
        if manual_news_push_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(manual_news_push_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{manual_news_push_data[field][i]}', "
