...
class VideoCtrByPlacement(object):
    '''
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ['country_code', 'date', 'placement', 'click_count', 'impression_count', 'ctr']
    ...
    def compute_data(self, path):
        ...
        video_ctr_by_placement_data = self.get_data(query)
        if video_ctr_by_placement_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(video_ctr_by_placement_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{video_ctr_by_placement_data[field][i]}", '
