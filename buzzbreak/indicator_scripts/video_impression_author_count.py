...
class VideoImpressionAuthorCount(object):
    """
    ...
    def compute_data(self, path):
        ...
        if video_impression_author_count_data[self.fields[0]]:
            ...
            for i in range(len(video_impression_author_count_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{video_impression_author_count_data[field][i]}', "
