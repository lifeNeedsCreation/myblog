...
class NewUserRetentionByBrand(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "initial_date", "retention_date", "date_diff", "brand", "initial_num", "retention_num", "retention_rate"]
    ...
    def compute_data(self, path):
        ...
        new_user_retention_by_brand_data = self.get_data(query)
        if new_user_retention_by_brand_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(new_user_retention_by_brand_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{new_user_retention_by_brand_data[field][i]}', "
