...
class VideoCtrRoughRankByModel(object):
    """
    ...
    def compute_data(self, path):
        ...
        video_ctr_rough_rank_by_model_data = self.get_data(query)
        if video_ctr_rough_rank_by_model_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(video_ctr_rough_rank_by_model_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{video_ctr_rough_rank_by_model_data[field][i]}', "
