...
class ThirtySecondsNewUserAppOpenRetention(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "initial_date", "retention_date", "date_diff", "initial_num", "new_user_total_num", "ratio", "retention_num", "retention_rate"]
    ...
    def compute_data(self, path):
        ...
        thirty_seconds_new_user_app_open_retention_data = self.get_data(query)
        if thirty_seconds_new_user_app_open_retention_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(thirty_seconds_new_user_app_open_retention_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{thirty_seconds_new_user_app_open_retention_data[field][i]}', "
