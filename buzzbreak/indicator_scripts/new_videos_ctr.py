...
class NewVideosCtr(object):
    ...
    def compute_data(self, path):
        ...
        new_videos_ctr_data = self.get_data(query)
        if new_videos_ctr_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(new_videos_ctr_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{new_videos_ctr_data[field][i]}", '
