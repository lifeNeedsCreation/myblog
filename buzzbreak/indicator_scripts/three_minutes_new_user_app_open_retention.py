...
class ThreeMinutesNewUserAppOpenRetention(object):
    """
    ...
    def compute_data(self, path):
        ...
        three_minutes_new_user_app_open_retention_data = self.get_data(query)
        if three_minutes_new_user_app_open_retention_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(three_minutes_new_user_app_open_retention_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{three_minutes_new_user_app_open_retention_data[field][i]}', "
