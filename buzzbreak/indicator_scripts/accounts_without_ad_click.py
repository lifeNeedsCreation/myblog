...
class AccountsWithoutAdClick(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "date", "ad_click_user_num", "ad_click_ratio", "without_ad_click_user_num", "without_ad_click_ratio", "active_user_num"]
    ...
    def compute_data(self, path):
        ...
        accounts_without_ad_click_data = self.get_data(query)
        if accounts_without_ad_click_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(accounts_without_ad_click_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{accounts_without_ad_click_data[field][i]}', "
