...
class SensitiveAndViolationComments(object):
    '''
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ['country_code', 'date', 'comment_num', 'comment_people_num', 'sensitive_words_num', 'violation_words_num', 'one_level_num', 'two_level_num', 'three_level_num', 'four_level_num', 'one_level_all_num', 'two_level_all_num', 'three_level_all_num', 'four_level_all_num']
    ...
    def compute_data(self, path):
        ...
        sensitive_and_violation_comments_data = self.get_data(query)
        if sensitive_and_violation_comments_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(sensitive_and_violation_comments_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{sensitive_and_violation_comments_data[field][i]}", '
