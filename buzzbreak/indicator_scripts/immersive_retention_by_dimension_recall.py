...
class ImmersiveRetentionByDimensionRecall(object):
    """
    ...
    """
    # 构造函数，初始化数据
    def __init__(self, start_time, end_time, experiments, table_name, logger=None):
    ...
    def compute_data(self, path):
        ...
        sql = read_sql(path)
        params = {"start_time": start_time, "end_time": end_time, "experiments": self.experiments}
        query = sql.format(**params)
        immersive_retention_by_dimension_recall_data = self.get_data(query)
        if immersive_retention_by_dimension_recall_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(immersive_retention_by_dimension_recall_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{immersive_retention_by_dimension_recall_data[field][i]}', "
