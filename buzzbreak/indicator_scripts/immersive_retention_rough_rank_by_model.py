...
class ImmersiveRetentionRoughRankByModel(object):
    """
    ...
    def compute_data(self, path):
        ...
        immersive_retention_rough_rank_by_model_data = self.get_data(query)
        if immersive_retention_rough_rank_by_model_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(immersive_retention_rough_rank_by_model_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{immersive_retention_rough_rank_by_model_data[field][i]}', "
