...
class NewUserVideoCTRPeopleData(object):
    ...
    def compute_data(self):
        ...
        click_data = self.get_data("select placement, key, country_code, value,count(*) as num from "
                                   ...
                                   "(select account_id, created_at, json_extract_scalar(data, '$.placement') as placement from buzzbreak-model-240306.stream_events.video_click where created_at>='" + self.start_time.strftime("%Y-%m-%d %H:%M:%S") + "' and created_at<'" + time_limit.strftime("%Y-%m-%d %H:%M:%S") + "') as click_data "
        ...
        impression_data = self.get_data("select placement, key, country_code, value,count(*) as num from "
                                        ...
                                        "(select account_id, created_at, json_extract_scalar(data, '$.placement') as placement from buzzbreak-model-240306.stream_events.video_impression where created_at>='" + self.start_time.strftime("%Y-%m-%d %H:%M:%S") + "' and created_at<'" + time_limit.strftime("%Y-%m-%d %H:%M:%S") + "') as impression_data"
        ...
        # 结果数据存入数据库
        insert_sql = "INSERT INTO " + self.table_name + " (treatment_name, placement, country_code, dimension, click_num, impression_num, impression_union_num, ctr, ctr_union, start_time, end_time, create_time) VALUES"
        ...
        for key in impression_data_union.keys():
            ...
            if len(temp_data) < 4:
                continue
            insert_sql = insert_sql + " ('" + temp_data[1] + "','" + temp_data[0] + "','" + temp_data[2] + "','" + temp_data[3] + "'," + str(click_num) + "," + str(impression_num) + "," + str(impression_num_union) + "," + str(round(click_num/impression_num, 5)) + "," + str(round(click_num/impression_num_union, 5)) + ",'" + self.start_time.strftime("%Y-%m-%d %H:%M:%S") + "','" + self.end_time.strftime("%Y-%m-%d %H:%M:%S") + "','" + now_time_utc.strftime("%Y-%m-%d %H:%M:%S") + "'),"
            flag = True
