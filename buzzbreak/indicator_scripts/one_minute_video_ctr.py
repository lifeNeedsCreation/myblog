...
class OneMinuteVideoCtr(object):
    """
    ...
    def compute_data(self, path):
        ...
        one_minute_video_ctr_data = self.get_data(query)
        if one_minute_video_ctr_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(one_minute_video_ctr_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{one_minute_video_ctr_data[field][i]}', "
