...
class NewsReadDurationAvgByCount(object):
    '''
    ...
    def compute_data(self, path):
        ...
        news_read_duration_avg_by_count_data = self.get_data(query)
        if news_read_duration_avg_by_count_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(news_read_duration_avg_by_count_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{news_read_duration_avg_by_count_data[field][i]}", '
