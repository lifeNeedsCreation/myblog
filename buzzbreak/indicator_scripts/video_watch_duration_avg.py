...
class VideoWatchDurationAvg(object):
    '''
    ...
    def compute_data(self, path):
        ...
        video_watch_duration_avg_data = self.get_data(query)
        if video_watch_duration_avg_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(video_watch_duration_avg_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{video_watch_duration_avg_data[field][i]}", '
