...
class CashOutByMethod(object):
    ...
    def compute_data(self, path):
        ...
        cashout_by_method_data = self.get_data(query)
        if cashout_by_method_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(cashout_by_method_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{cashout_by_method_data[field][i]}', "
