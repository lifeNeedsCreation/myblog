...
class VideoAverageOfTotalDurationRank(object):
    """
    ...
    def compute_data(self, path):
        ...
        duration_avg_rank_data = self.get_data(query)
        if duration_avg_rank_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(duration_avg_rank_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{duration_avg_rank_data[field][i]}', "
