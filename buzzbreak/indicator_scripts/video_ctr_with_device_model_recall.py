...
class VideoCtrWithDeviceModelRecall(object):
    """
    ...
    def compute_data(self, path):
        ...
        if video_ctr_with_device_model_recall_data[self.fields[0]]:
            ...
            for i in range(len(video_ctr_with_device_model_recall_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f'"{video_ctr_with_device_model_recall_data[field][i]}", '
                insert_sql += f"'{now_time_utc}'),"
                if i % n == 0:
                    insert_sql1 = insert_sql + value_sql[:-1]
                    buzzbreak_mysql_client.execute_sql(insert_sql1)
                    value_sql = ''
                elif (i - len(video_ctr_with_device_model_recall_data[self.fields[0]])) == 0:
