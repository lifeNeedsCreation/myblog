...
class CTRPeopleData(object):
    ...
    def compute_data(self):
        click_data = self.get_data("select placement, key, country_code, value,count(*) as num from "
                                   ...
                                   "(select account_id, created_at, json_extract_scalar(data, '$.placement') as placement from buzzbreak-model-240306.stream_events.news_click where created_at>='" + self.start_time.strftime("%Y-%m-%d %H:%M:%S") + "' and created_at<'" + self.end_time.strftime("%Y-%m-%d %H:%M:%S") + "') as click_data "
        ...
        impression_data = self.get_data("select placement, key, country_code, value,count(*) as num from "
                                        ...
                                        "(select account_id, created_at, json_extract_scalar(data, '$.placement') as placement from buzzbreak-model-240306.stream_events.news_impression where created_at>='" + self.start_time.strftime("%Y-%m-%d %H:%M:%S") + "' and created_at<'" + self.end_time.strftime("%Y-%m-%d %H:%M:%S") + "') as impression_data"
