...
class PageDauAndPenetration(object):
    '''
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ['country_code', 'date', 'page', 'page_count', 'app_open_count', 'penetration_rate']
    ...
    def compute_data(self, path):
        ...
        page_dau_and_penetration_data = self.get_data(query)
        if page_dau_and_penetration_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(page_dau_and_penetration_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{page_dau_and_penetration_data[field][i]}", '
