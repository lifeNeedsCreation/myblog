...
class PushCtrWithoutExperiments(object):
    """
    ...
    def compute_data(self, path):
        ...
        push_ctr_without_experiment_data = self.get_data(query)
        if push_ctr_without_experiment_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(push_ctr_without_experiment_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{push_ctr_without_experiment_data[field][i]}', "
