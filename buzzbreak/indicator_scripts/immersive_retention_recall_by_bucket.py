...
class ImmersiveRetentionRecallByBucket(object):
    """
    ...
    def __init__(self, start_time, end_time, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "placement", "bucket", "initial_date", "retention_date", "date_diff", "initial_num", "retention_num", "retention_rate"]
    ...
    def compute_data(self, path):
        ...
        immersive_retention_recall_by_bucket_data = self.get_data(query)
        if immersive_retention_recall_by_bucket_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(immersive_retention_recall_by_bucket_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{immersive_retention_recall_by_bucket_data[field][i]}', "
