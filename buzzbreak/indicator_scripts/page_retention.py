...
class PageRetention(object):
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ['country_code', 'page', 'initial_date', 'retention_date', 'date_diff', 'initial_count', 'retention_count', 'retention_rate']
    ...
    def compute_data(self, path):
        ...
        image_ctr_by_placement_data = self.get_data(query)
        if image_ctr_by_placement_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(image_ctr_by_placement_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{image_ctr_by_placement_data[field][i]}", '
