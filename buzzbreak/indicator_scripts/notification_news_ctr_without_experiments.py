...
class NotificationNewsCtrWithoutExperiments(object):
    '''
    ...
    def compute_data(self, path):
        ...
        notification_news_ctr_without_experiments_data = self.get_data(query)
        if notification_news_ctr_without_experiments_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(notification_news_ctr_without_experiments_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{notification_news_ctr_without_experiments_data[field][i]}", '
