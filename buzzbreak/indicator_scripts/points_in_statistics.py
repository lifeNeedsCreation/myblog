...
class PointsInStatistics(object):
    """
    ...
    def compute_data(self, path):
        ...
        points_in_statistics_data = self.get_data(query)
        if points_in_statistics_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(points_in_statistics_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{points_in_statistics_data[field][i]}', "
