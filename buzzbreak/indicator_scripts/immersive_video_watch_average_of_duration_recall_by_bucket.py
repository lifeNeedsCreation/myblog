...
class ImmersiveVideoWatchAverageOfDurationRecallByBucket(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "placement", "bucket", "date", "user_num", "duration_sum", "duration_avg"]
    ...
    def compute_data(self, path):
        ...
        immersive_video_watch_average_of_duration_recall_by_bucket_data = self.get_data(query)
        if immersive_video_watch_average_of_duration_recall_by_bucket_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(immersive_video_watch_average_of_duration_recall_by_bucket_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{immersive_video_watch_average_of_duration_recall_by_bucket_data[field][i]}', "
