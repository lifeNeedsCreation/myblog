...
class PartikoMemoriesNewUsersEventsRetentionWithImpression(object):
    ...
    def compute_data(self, path):
        ...
        if partiko_memories_new_user_events_retention_news_and_video_impression_data[self.fields[0]]:
            ...
            for i in range(len(partiko_memories_new_user_events_retention_news_and_video_impression_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{partiko_memories_new_user_events_retention_news_and_video_impression_data[field][i]}', "
