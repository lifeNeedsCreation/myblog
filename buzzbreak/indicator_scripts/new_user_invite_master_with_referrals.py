...
class NewUserInviteMasterWithReferrals(object):
    """
    ...
    def compute_data(self, path):
        ...
        new_user_invite_master_with_referrals_data = self.get_data(query)
        if new_user_invite_master_with_referrals_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(new_user_invite_master_with_referrals_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{new_user_invite_master_with_referrals_data[field][i]}', "
