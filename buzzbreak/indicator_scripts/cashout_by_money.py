...
class CashOutByMoney(object):
    ...
    def __init__(self, start_time, end_time, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "paid_out_date", "money", "payout_method", "cashout_num", "user_num"]
    ...
    def compute_data(self, path):
        ...
        cashout_by_money_data = self.get_data(query)
        if cashout_by_money_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(cashout_by_money_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{cashout_by_money_data[field][i]}', "
