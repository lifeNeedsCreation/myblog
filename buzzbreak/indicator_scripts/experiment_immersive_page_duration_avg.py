...
class ExperimentImmersivePageDurationAvg(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "treatment_name", "indimension", "page", "date", "duration_sum", "user_count", "duration_avg"]
    ...
    def compute_data(self, path):
        ...
        duration_age_data = self.get_data(query)
        if duration_age_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(duration_age_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{duration_age_data[field][i]}', "
