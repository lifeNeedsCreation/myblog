...
class VideoRetentionRecall(object):
    """
    ...
    def __init__(self, start_time, end_time, country_code, experiments, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "placement", "initial_date", "retention_date", "date_diff", "experiment", "strategy", "initial_num", "retention_num", "retention_rate"]
    ...
    def compute_data(self, path):
        ...
        video_retention_recall_data = self.get_data(query)
        if video_retention_recall_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(video_retention_recall_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{video_retention_recall_data[field][i]}', "
