...
class ShortNewVideoCompletionRate(object):
    """
    ...
    def compute_data(self, path):
        ...
        short_new_video_completion_rate_data = self.get_data(query)
        if short_new_video_completion_rate_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(short_new_video_completion_rate_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{short_new_video_completion_rate_data[field][i]}', "
