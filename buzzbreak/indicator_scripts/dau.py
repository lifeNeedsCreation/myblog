...
class DAU(object):
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ['country_code', 'date', 'visitor_num', 'user_num', 'total_user_num']
    ...
    def compute_data(self, path):
        ...
        dau_data = self.get_data(query)
        if dau_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ''
            for field in self.fields:
                values += field + ', '
            ...
            for i in range(len(dau_data[self.fields[0]])):
                insert_sql += '('
                for field in self.fields:
                    insert_sql += f'"{dau_data[field][i]}", '
