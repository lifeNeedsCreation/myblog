...
class SilentUser(object):
    ...
    def __init__(self, start_time, end_time, country_code, table_name, logger=None):
        ...
        self.logger = logger
        self.fields = ["country_code", "date", "one_level_inactive_user_num", "one_level_ratio", "two_level_inactive_user_num", "two_level_ratio", "three_level_inactive_user_num", "three_level_ratio", "total_user_num"]
    ...
    def compute_data(self, path):
        ...
        silent_user_data = self.get_data(query)
        if silent_user_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(silent_user_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{silent_user_data[field][i]}', "
