...
class TenMinutesNewUserAppOpenRetention(object):
    """
    ...
    def compute_data(self, path):
        ...
        ten_minutes_new_user_app_open_retention_data = self.get_data(query)
        if ten_minutes_new_user_app_open_retention_data[self.fields[0]]:
            # 结果数据存入数据库
            values = ""
            for field in self.fields:
                values += field + ", "
            ...
            for i in range(len(ten_minutes_new_user_app_open_retention_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{ten_minutes_new_user_app_open_retention_data[field][i]}', "
