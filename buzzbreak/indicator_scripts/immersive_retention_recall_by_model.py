...
class ImmersiveRetentionRecallByModel(object):
    """
    ...
    def compute_data(self, path):
        ...
        if immersive_retention_recall_by_model_data[self.fields[0]]:
            ...
            for i in range(len(immersive_retention_recall_by_model_data[self.fields[0]])):
                insert_sql += "("
                for field in self.fields:
                    insert_sql += f"'{immersive_retention_recall_by_model_data[field][i]}', "
