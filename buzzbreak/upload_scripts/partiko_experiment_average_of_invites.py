...
class PartikoExperimentAverageOfInvites:
    # 构造函数， 初始化数据
    """
    ...
    def compute_data(self):
        ...
        query = \
            f'''
            with
            experiments as (select * from partiko.experiments where group_field in ({self.indicator_dimension})),
            ...
            experiment_accounts as (select distinct id,country_code,key,group_field,experiments.created_at as experiment_timestamp from accounts inner join experiments on id=account_id),
            experiment_referrals as (select distinct id,referee_account_id,country_code,key,group_field,experiment_timestamp,created_at from (select distinct referrer_account_id, referee_account_id, created_at from referrals) inner join experiment_accounts on referrer_account_id=id where created_at>experiment_timestamp),
            experiment_app_open as (select distinct id,country_code,key,group_field,date from (select distinct account_id,extract(date from created_at) as date from app_open) inner join experiment_accounts on account_id=id where date>=extract(date from experiment_timestamp))
            select o.country_code,o.key,o.group_field,o.date,open_users_count,ifnull(referees_count,0) as referees_count,round(ifnull(referees_count, 0)/open_users_count,4) as avg_referees_count from (select country_code,key,group_field,extract(date from created_at) as date,count(distinct referee_account_id) as referees_count from experiment_referrals group by country_code,key,group_field,date) as r right join (select country_code,key,group_field,date,count(distinct id) as open_users_count from experiment_app_open group by country_code,key,group_field,date) as o on r.country_code=o.country_code and r.key=o.key and r.group_field=o.group_field and r.date=o.date
            '''
