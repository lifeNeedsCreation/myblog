...
class NewUserNewsClickAverageData(object):
    ...
    def compute_data(self):
        ...
        news_click_sql = \
            ...
            where news_click.created_at > '{start_time}' and news_click.created_at < '{end_time}' and json_extract_scalar(data, '$.placement') in ({self.placement})) as news_click
        ...
        impression_sql = \
            ...
            (select account_id, created_at, json_extract_scalar(data, '$.placement') as placement from `stream_events.news_impression` as news_impression where news_impression.created_at > '{start_time}' and news_impression.created_at < '{end_time}' and json_extract_scalar(data, '$.placement') in ({self.placement})) as news_impression
        ...
        impression_union_sql = \
            ...
            (select account_id, created_at, json_extract_scalar(data, '$.placement') as placement from `stream_events.news_click` as news_click where news_click.created_at > '{start_time}' and news_click.created_at < '{end_time}' and json_extract_scalar(data, '$.placement') in ({self.placement})) as news_click
        ...
        for key in impression_union_data.keys():
            ...
            if len(temp_data) < 4:
                continue
            values_sql = "('" + temp_data[0] + "','" + temp_data[1] + "','" + temp_data[2] + "','" + temp_data[3] + "'," + str(click_num) + "," + str(impression_num) + "," + str(impression_num_union) + "," + str(round(click_num/impression_num, 5)) + "," + str(round(click_num/impression_num_union, 5)) + ",'" + start_time + "','" + end_time + "','" + now_time_utc + "'),"
